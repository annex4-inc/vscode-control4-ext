...
export abstract class SveltePanel {
  protected _viewType: string;

  protected readonly _svelteScript: string;
  ...
  public async show(entity) {
    console.log(entity)
    try {
      this._panel.webview.postMessage({ command: "update", entity: entity })
    } catch (err) {
      console.log(err)
    }
  }
  ...
  protected constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, svelteScript: string) {
    this._panel = panel;
